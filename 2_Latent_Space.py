...
def scatter(DATA, COLOR_FEATURES, SEL_GENES, key, COLOR_DICTS=None, default_index=0, 
        ...
        with st.popover("", icon=":material/settings:"):
            ...
            if not CAT:
                ...
                selected_range = st.slider(
                    "Select Range", min_value=min_val, max_value=max_val,
    ...
    fig.update_traces(marker=dict(size=point_size, opacity=point_opacity, color=colors))
        ...
        for gene in SEL_GENES:
            if gene in DATA.index:
                ...
                fig.add_trace(go.Scattergl(
                    ...
                    showlegend=True,  # Avoid duplicate legend entries
                    hovertemplate=(
                        f"<b>Gene:</b> {gene}<br>"
